...
import type { ListResponse, ProviderFallback } from "@workspace/api-client/types"
...
export function useProviderFallbacks() {
  ...
  return useQuery({
    ...
    queryFn: () => api.get<ListResponse<ProviderFallback>>("/api/provider-fallbacks"),
  })
}
...
export function useCreateProviderFallback() {
  ...
  return useMutation({
    mutationFn: (body: { channel: string; primaryConnectionId: string; fallbackConnectionId: string }) =>
