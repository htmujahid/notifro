...
function parse(tokens: Token[]): Node[] {
  ...
  function parseNodes(
    stopAt: "endif" | "endeach" | "else" | null = null
  ): Node[] {
    const nodes: Node[] = []
    ...
    while (pos < tokens.length) {
      ...
      if (
        stopAt &&
        (tok.type === stopAt ||
          (stopAt === "else" && (tok.type === "else" || tok.type === "endif")))
      ) {
        break
      }
      if (
...
function evaluateNodes(
  ...
  for (const node of nodes) {
    if (out.value.length > MAX_OUTPUT_BYTES)
      throw new Error("Template output exceeds size limit")
    ...
    if (node.type === "text") {
      ...
      evaluateNodes(
        isTruthy(val) ? node.then : node.else,
        ctx,
        localeStrings,
        out
      )
      ...
      if (Array.isArray(items)) {
        ...
        for (let i = 0; i < len; i++) {
          ...
          evaluateNodes(
            node.body,
            itemCtx as Record<string, unknown>,
            localeStrings,
            out
          )
        }
