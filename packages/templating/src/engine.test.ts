...
describe("renderString — variables", () => {
  ...
  test("resolves nested paths", () => {
    assert.equal(
      renderString("Hi {{user.first}}", { user: { first: "Bob" } }),
      "Hi Bob"
    )
  })
  ...
  test("uses default when variable is undefined", () => {
    assert.equal(
      renderString('{{x | "fallback"}}', { x: undefined }),
      "fallback"
    )
  })
...
describe("renderString — conditionals", () => {
  ...
  test("renders else-branch when falsy", () => {
    assert.equal(
      renderString("{{#if ok}}a{{else}}b{{/if}}", { ok: false }),
      "b"
    )
  })
  ...
  test("handles nested path in condition", () => {
    assert.equal(
      renderString("{{#if user.premium}}VIP{{/if}}", {
        user: { premium: true },
      }),
      "VIP"
    )
  ...
  test("treats empty array as falsy", () => {
    assert.equal(
      renderString("{{#if items}}has{{else}}empty{{/if}}", { items: [] }),
      "empty"
    )
  })
  ...
  test("treats non-empty array as truthy", () => {
    assert.equal(
      renderString("{{#if items}}has{{else}}empty{{/if}}", { items: [1] }),
      "has"
    )
  })
})
...
describe("renderString — loops", () => {
  ...
  test("exposes itemIndex", () => {
    assert.equal(
      renderString("{{#each items as i}}{{iIndex}}{{/each}}", {
        items: ["x", "y"],
      }),
      "01"
    )
  ...
  test("skips loop for empty array", () => {
    assert.equal(
      renderString("{{#each items as i}}{{i}}{{/each}}", { items: [] }),
      ""
    )
  })
...
describe("renderValue — JSON walk", () => {
  ...
  test("renders strings inside arrays", () => {
    assert.deepEqual(
      renderValue(["Hi {{a}}", "Bye {{b}}"], { a: "X", b: "Y" }),
      ["Hi X", "Bye Y"]
    )
  })
  ...
  test("handles nested objects recursively", () => {
    ...
    assert.deepEqual(renderValue(tpl, { msg: "Hello" }), {
