...
import { CopyIcon, PlusIcon, TrashIcon, CodeIcon, KeyIcon, ScrollIcon, RefreshCwIcon, BotIcon, CheckIcon, XIcon } from "lucide-react"
import { useApiKeys, useCreateApiKey, useRevokeApiKey, useRequestLog } from "@workspace/core/hooks/developers"
import { useMcpGates, useUpsertMcpGate, useDeleteMcpGate, useMcpPending, useApproveMcpAction, useRejectMcpAction } from "@workspace/core/hooks/mcp"
...
const MCP_TOOLS = ['send_notification', 'schedule_notification', 'create_template']

function McpSection() {
  const api = useApiClient()
  const { data: gatesData, isLoading: gatesLoading } = useMcpGates()
  const { data: pendingData, isLoading: pendingLoading, refetch: refetchPending } = useMcpPending()
    ...
    <section className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">Connect AI agents via the Model Context Protocol.</p>
        </div>
      ...
      <Card size="sm">
        <CardContent className="flex flex-col gap-3">
          <div>
            <p className="text-xs font-medium text-muted-foreground mb-1">Remote endpoint</p>
            <div className="flex items-center gap-2">
              <code className="flex-1 rounded bg-muted px-2 py-1 text-xs font-mono break-all">{mcpUrl}</code>
              <Button
                size="icon"
                variant="outline"
                className="size-7 shrink-0"
            ...
            <p className="mt-1.5 text-xs text-muted-foreground">Authenticate with an API key: <code className="text-xs">Authorization: Bearer rk_live_…</code></p>
          </div>

          <div>
            <p className="text-xs font-medium text-muted-foreground mb-1">Local stdio binary</p>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-3">When enabled, outbound actions require human approval before executing.</p>
        {gatesLoading ? (
          <p className="text-xs text-muted-foreground">Loading…</p>
        ) : (
          <div className="flex flex-col divide-y rounded-lg border">
            {MCP_TOOLS.map((tool) => {
              ...
              return (
                <div key={tool} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <Badge variant={requires ? "default" : "secondary"} className="text-xs">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <Button size="sm" variant="outline" className="h-6 text-xs px-2 gap-1" onClick={() => refetchPending()}>
