...
interface UseCase {
  ...
  desc: string;
  points: string[];
  rows: { channel: string; status: string; tone: "delivered" | "opened" | "sent" }[];
}
...
const cases: UseCase[] = [
  {
    ...
    points: ["Idempotency keys prevent duplicates", "push → email → SMS fallback", "Delivery receipts & bounce handling"],
    rows: [
  ...
  },
  {
    ...
    points: ["Priority-based routing rules", "Quiet hours & DND windows", "Throttle/debounce to avoid storms"],
    rows: [
  ...
  },
  {
    ...
    points: ["Multi-step journeys with delays", "A/B testing & dynamic content", "Send-time optimization per recipient"],
    rows: [
      { channel: "Email", status: "Opened", tone: "opened" },
  ...
  },
  {
    ...
    points: ["Scoped MCP tools for agents", "Preview & approval gates", "Every send fully audited"],
    rows: [
      { channel: "Slack", status: "Delivered", tone: "delivered" },
  ...
  return (
    <Tabs defaultValue="transactional" className="w-full">
      <TabsList className="mx-auto flex h-auto flex-wrap justify-center gap-1 p-1">
        {cases.map((c) => (
          <TabsTrigger key={c.id} value={c.id} className="flex-none gap-1.5 px-3 py-1.5">
          ...
          <div className="grid items-center gap-10 lg:grid-cols-2">
            ...
            <div className="rounded-xl border bg-card p-4">
              <div className="flex items-center gap-2 border-b px-1 pb-3">
                ...
                <span className="size-2.5 rounded-full bg-muted-foreground/20" />
                <span className="size-2.5 rounded-full bg-muted-foreground/20" />
                <span className="ml-2 font-mono text-xs text-muted-foreground">delivery · {c.id}</span>
              </div>
              <div className="divide-y">
                {c.rows.map((r) => (
                  <div key={r.channel} className="flex items-center justify-between py-3">
                    <span className="text-sm font-medium">{r.channel}</span>
                    <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${toneClasses[r.tone]}`}>
                      {r.status}
                    </span>
                  </div>
                ))}
