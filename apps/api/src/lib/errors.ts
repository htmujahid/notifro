...
export function validationHook(result: { success: boolean; error?: unknown }, c: any): Response | void {
  if (!result.success) {
    return c.json({ error: { code: 'validation_error', message: 'Validation failed', details: result.error } }, 422)
  }
}
...
export const Errors = {
  ...
  validationError: (details?: unknown) => new ApiError(422, 'validation_error', 'Validation failed', details),
