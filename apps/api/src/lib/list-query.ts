...
export function applyListQuery<T>(
  ...
  const order =
    (parsed.order as "asc" | "desc" | undefined) ?? config.defaultSort.order
  ...
  for (const [paramKey, spec] of Object.entries(config.filterable)) {
    ...
    else if (spec.operator === "like")
      builder = builder.where(col, "like", `%${value}%`)
    else if (spec.operator === "not_like")
      builder = builder.where(col, "not like", `%${value}%`)
  ...
  function getPage<R extends Record<string, unknown>>(rows: R[]) {
    ...
    const nextCursor =
      hasMore && last
        ? encodeCursor({
            v: last[sortCol.split(".").pop()!] ?? last[sortKey],
