...
function evaluateFilter(
  ...
  if ("and" in node) {
    return (node as { and: FilterNode[] }).and.every((n) =>
      evaluateFilter(n, context)
    )
  }
  if ("or" in node) {
    return (node as { or: FilterNode[] }).or.some((n) =>
      evaluateFilter(n, context)
    )
  }
  ...
  switch (c.op) {
    ...
    case "gt":
      return (
        typeof val === "number" && typeof c.value === "number" && val > c.value
      )
    case "lt":
      return (
        typeof val === "number" && typeof c.value === "number" && val < c.value
...
export async function advanceJourneyRun(
  ...
  if (step.kind === "branch") {
    const branches =
      (step.config.branches as { condition: FilterNode; next: string }[]) ?? []
