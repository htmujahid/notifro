  ...
  if (secret) {
    ...
    if (claims && claims.t === 'open') {
      ...
      if (delivery && !delivery.openedAt) {
        await db.updateTable('delivery').set({ openedAt: ts, updatedAt: ts }).where('id', '=', claims.d).execute()
        await db
          .insertInto('delivery_event')
          .values({ id: crypto.randomUUID(), deliveryId: claims.d, userId: delivery.userId, type: 'opened', at: ts, meta: '{}' })
          .execute()
      }
    }
  }
  ...
  if (secret) {
    ...
    if (claims && claims.t === 'click' && claims.u) {
      ...
      const delivery = await db
        ...
        .executeTakeFirst()
      if (delivery) {
        await db.updateTable('delivery').set({ clickedAt: ts, updatedAt: ts }).where('id', '=', claims.d).execute()
        await db
          .insertInto('delivery_event')
          .values({
