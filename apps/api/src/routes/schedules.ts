...
const listRoute = createRoute({
  ...
  responses: {
    200: { content: { 'application/json': { schema: ListResponseSchema } }, description: 'Paginated scheduled messages' },
  },
})
...
const deleteRoute = createRoute({
  ...
  path: '/schedules/:id',
  responses: {
    200: { content: { 'application/json': { schema: z.object({ ok: z.literal(true) }) } }, description: 'Cancelled' },
  },
})
...
router.openapi(listRoute, async (c) => {
  ...
  return c.json({ data: page.data as z.infer<typeof ScheduledMessageDtoSchema>[], nextCursor: page.nextCursor })
})
...
router.openapi(deleteRoute, async (c) => {
  ...
  if (msg.status !== 'pending') throw Errors.badRequest('Only pending messages can be cancelled')
