...
export default router.post("/:provider", async (c) => {
  ...
  if (provider === "twilio") {
    ...
    if (!messageSid || !messageStatus)
      return c.json(
        { ok: false, error: "Missing MessageSid or MessageStatus" },
        400
      )
    ...
    if (messageStatus === "delivered") {
      await db
        ...
        .set({ deliveredAt: ts, updatedAt: ts })
        .where("id", "=", delivery.id)
        .execute()
      await recordEvent(db, delivery.id, delivery.userId, "delivered", ts, {
        messageStatus,
      })
    } else if (messageStatus === "undelivered" || messageStatus === "failed") {
      await db
        .updateTable("delivery")
        .set({ bouncedAt: ts, status: "bounced", updatedAt: ts })
        .where("id", "=", delivery.id)
        .execute()
