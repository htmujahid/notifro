...
function blockToText(block: ContentBlock): string | null {
  switch (block.type) {
    ...
    case 'button_group':
      return block.buttons.map((b) => (b.url ? `[${b.label}](${b.url})` : b.label)).join(' · ')
    case 'divider':
      return '---'
    default:
      return null
...
const discordAdapter: ChannelAdapter<DiscordConfig, DiscordProvider> = {
  ...
  transform(payload, { connection }): DiscordProvider {
    ...
    for (const block of content.blocks ?? []) {
      if (block.type === 'fields') {
        for (const f of block.fields) fields.push({ name: f.key, value: f.value, inline: true })
        continue
      }
      if (block.type === 'image') {
  ...
  async send(provider, conn, ctx) {
    ...
    if (!conn.credentials) {
      return { providerMessageId: null, ok: false, error: 'No Discord webhook URL — add credentials.webhookUrl' }
    }

    let creds: DiscordCredentials
    try {
      creds = JSON.parse(await decrypt(conn.credentials, encKey)) as DiscordCredentials
    } catch {
      return { providerMessageId: null, ok: false, error: 'Failed to decrypt Discord credentials' }
    }
    if (!creds.webhookUrl) {
      return { providerMessageId: null, ok: false, error: 'Discord credentials missing webhookUrl' }
    }
    ...
    try {
      ...
      return { providerMessageId: null, ok: false, error: 'Discord webhookUrl is not a valid URL' }
    }
      ...
      if (res.status < 200 || res.status >= 300) {
        ...
        if (res.status === 429) {
          try {
            ...
            if (j.retry_after != null) detail = `HTTP 429 rate limited (retry_after ${j.retry_after}s)`
          } catch {}
        }
  ...
  async healthCheck(conn) {
    if (!conn.credentials) {
      return { ok: false, message: 'No webhook URL — add credentials.webhookUrl', checkedAt: new Date().toISOString() }
    }
    return { ok: true, message: 'Webhook URL present', checkedAt: new Date().toISOString() }
  },
}
