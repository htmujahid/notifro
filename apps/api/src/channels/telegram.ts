...
const telegramAdapter: ChannelAdapter<TelegramConfig, TelegramProvider> = {
  ...
  async send(provider, conn, ctx) {
    ...
    if (!conn.credentials) {
      return { providerMessageId: null, ok: false, error: 'No Telegram bot token — add credentials.botToken' }
    }

    let creds: TelegramCredentials
    try {
      creds = JSON.parse(await decrypt(conn.credentials, encKey)) as TelegramCredentials
    } catch {
      return { providerMessageId: null, ok: false, error: 'Failed to decrypt Telegram credentials' }
    }
    if (!creds.botToken) {
      return { providerMessageId: null, ok: false, error: 'Telegram credentials missing botToken' }
    }
