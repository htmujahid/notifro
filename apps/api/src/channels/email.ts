  ...
  async send(provider, conn, ctx) {
    ...
    if (secret && baseUrl && ctx.recipientId) {
      ...
      if (recipientRow) {
        ...
        unsubscribeToken = await signPreferenceToken({ recipientId: ctx.recipientId, userId: recipientRow.userId, exp }, secret)
      }
    }
    ...
    const emailHeaders: Record<string, string> | undefined = unsubscribeToken && baseUrl
      ? {
