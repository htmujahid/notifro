...
export async function buildVapidHeaders(
  ...
  const payload = bytesToB64url(
    new TextEncoder().encode(JSON.stringify({ aud: origin, exp: now + 43200, sub: subject })),
  )
  const toSign = `${header}.${payload}`

  const sigBuf = await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, signingKey, new TextEncoder().encode(toSign))
