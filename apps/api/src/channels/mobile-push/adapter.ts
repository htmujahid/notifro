...
import { type ApnsCredentials, sendApns } from "./apns"
import { type FcmCredentials, sendFcm } from "./fcm"
...
const mobilePushAdapter: ChannelAdapter<
  Record<string, never>,
  MobilePushProvider
> = {
  type: "mobile_push",
  ...
  transform(payload: ComposePayload, { connection }): MobilePushProvider {
    ...
    const userId =
      recipient.type === "user" && recipient.userId
        ...
        : connection.userId
    const title =
      payload.content.title ?? payload.content.subject ?? "New notification"
  ...
  async send(provider, conn, ctx) {
    ...
    if (!conn.credentials) {
      return {
        providerMessageId: null,
        ok: false,
        error: "No mobile push credentials — add apns and/or fcm",
      }
    }
    ...
    try {
      ...
      ) as MobilePushCredentials
    } catch {
      return {
        providerMessageId: null,
        ok: false,
        error: "Failed to decrypt mobile push credentials",
      }
    }
    if (!creds.apns && !creds.fcm) {
      return {
        providerMessageId: null,
        ok: false,
        error: "Mobile push credentials missing apns and fcm",
      }
    }
    ...
    return {
      providerMessageId: null,
      ok: false,
      error: errors.join("; ") || "All mobile push sends failed",
    }
  },
  ...
  async healthCheck(conn) {
    if (!conn.credentials) {
      return {
        ok: false,
        message: "No credentials — add apns and/or fcm",
        checkedAt: new Date().toISOString(),
      }
    }
    return {
      ok: true,
      message: "Mobile push credentials present",
      checkedAt: new Date().toISOString(),
    }
  },
}
