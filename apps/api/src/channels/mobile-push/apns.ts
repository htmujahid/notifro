...
async function buildJwt(creds: ApnsCredentials): Promise<string> {
  ...
  const header = bytesToB64url(new TextEncoder().encode(JSON.stringify({ alg: 'ES256', kid: creds.keyId })))
  const payload = bytesToB64url(new TextEncoder().encode(JSON.stringify({ iss: creds.teamId, iat })))
...
export async function sendApns(
  ...
  return { ok: false, status: res.status, apnsId, reason: reason ?? `HTTP ${res.status}`, unregistered }
}
