function tzParts(date: Date, tz: string) {
  ...
  let h = get('hour')
  if (h === 24) h = 0
  return { year: get('year'), month: get('month'), day: get('day'), h, m: get('minute'), s: get('second') }
}
...
export function isInQuietHours(utcNow: Date, startHHMM: string, endHHMM: string, tz: string): boolean {
...
export function isInDeliveryWindow(utcNow: Date, windowStart: string, windowEnd: string, tz: string): boolean {
  const now = localMinutes(utcNow, tz)
...
export function nextAllowedTime(utcNow: Date, endHHMM: string, tz: string): Date {
  const [eh, em] = endHHMM.split(':').map(Number)
...
export function nextWindowStart(utcNow: Date, windowStartHHMM: string, tz: string): Date {
